...
module.exports = {
    name: "ml.bankaldır",
    description: "Hexi girilen kişiye.",
    execute(message, args, connection) {
        const ckEmbed = new Discord.MessageEmbed()
            .setFooter("dev. by Marvellous")
            if (message.member.hasPermission("KICK_MEMBERS")) {
                ...
                message.channel.awaitMessages(m => m.author.id === message.author.id, {
                    ...
                    if (c.first().content.toLowerCase() === "evet") {
                        connection.query("SELECT * FROM ml_banlist WHERE steam = ?",hex, (err,result) => {
                           let user = result[0]
                           if (user) {
                                connection.query("DELETE FROM ml_banlist WHERE steam = ?",hex, (err,results,fields) => {
                                })
                                ...
                                .setColor("GREEN")
                                .setDescription(`${hex} HEX ID'li kişinin başarıyla banı kaldırıldı`)
